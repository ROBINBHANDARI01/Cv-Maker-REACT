// src/Templates/Template2/index.jsx
import { useResume } from "../../context/ResumeContext";
import { themes } from "./theme";             // ← shared theme, same as Template4
...
function Template2({ themeId = "green", data }) {
  const { resume: contextResume } = useResume();  // ← same pattern as Template4
    ...
    <div className="bg-white text-gray-900 w-198.5 min-h-280.75 mx-auto">
      ...
      <div className="grid grid-cols-[2.2fr_1fr] gap-8 px-8 py-8">
          ...
          <div className="mt-4 space-y-6">
            {resume.projects.map((project, i) => (
              <div key={i}>
                ...
                {project.liveUrl && (
                  <p className="text-[10px] mt-2 text-gray-400">↗ {project.liveUrl}</p>
                )}
                {project.githubUrl && (            // ← was checking liveUrl twice before
                  <p className="text-[10px] mt-2 text-gray-400">↗ {project.githubUrl}</p>
                )}
              </div>
            ))}
          </div>
        ...
        <div
          ...
          style={{ borderLeft: `1px solid ${t.accent}20`, fontFamily: "sans-serif" }}
        >
          {/* SKILLS */}
            ...
            <div className="space-y-5 mt-4">
              {resume.education.map((edu, i) => (
                <div key={i}>
                  ...
                  <p className="text-[10px] text-gray-500 mt-1">{edu.educationPlace}</p>
                  <p
